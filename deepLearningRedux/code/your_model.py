...
class YourModel(ModelDesc):
    ...
    def _get_inputs(self):
        return [InputDesc(tf.float32, [None, 8, 8, 100], 'input'),
    ...
    def _build_graph(self, inputs):
        ...
        logits = Conv2D('conv0', image, 10, (2,2), padding='valid', stride=(1,1), nl=tf.nn.relu)
        logits = MaxPooling('pool0', logits, (2,2), stride=None, padding='valid')
        logits = Conv2D('conv1', logits, 10, (2,2), padding='valid', stride=(1,1), nl=tf.nn.relu)
        ...
        logits = FullyConnected('fc0', logits, 4, nl=tf.identity)
